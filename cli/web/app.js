...
function render() {
  ...
  for (const r of visible) {
    ...
    if (r.watch_log || (r.details || []).length) {
      ...
      det.textContent =
        bits.concat(r.details.slice(0, 4)).join("  路  ") +
        (r.details.length > 4 ? `  路  +${r.details.length - 4} more` : "");
      main.append(det);
    }
...
function parsedHtml(r) {
  ...
  if ((r.details || []).length) {
    rows.push(`<div class="parsed-row"><b>Directives</b>${r.details.map((d) => `路 ${d}`).join("<br>")}</div>`);
  }
  if (r.kind === "raw") {
